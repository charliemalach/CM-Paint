 ...
 * This class file is used to manage the changes made to individual tabs. This
 * class also manages multiple canvases in the canvas stack, allowing for
 * multiple canvases.
 **/
...
public class PaintTabs extends Tab {
    ...
    private void tabStart() { // default constructor
        ...
        this.setOnCloseRequest(new EventHandler<Event>() {
            ...
            public void handle(Event arg0) {
                quitTab(); // prompts the user to save before closing the current tab with the quitTab()
                           // method
            }
        });
     ...
     */
    public static void openImage() { // uses file chooser to open an image stored in path and displays it on the
                                     // canvas
    ...
    /**
     * 'Smart Save' function prompting users to save their work prior to closing the
     * program
     */
    ...
    /**
     * 'Smart Save' function that prompts users to save their work prior to closing
     * specific tab
     */
     ...
     * 
     * @param content - 'content' as string value of changes being made to the
     *                canvas
     */
    public static void logData(String content) { // logs the data to a new file
        try {
            File file = new File(log.toURI());
            ...
            bw.write("[" + LocalDate.now().toString() + "]" + "  " + "[" + LocalTime.now().toString() + "] "
                    + getCurrentTab().getText() + " -" + content + "\r\n");
            bw.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void resizeCanvas(int x) // resizes the entire canvas proportional with the newly provided canvas width
    {
        ...
        this.canvas.setHeight(x / 1.78); // sets the height. i use this to make it proportional. 1920 / 1080 = ~1.78,
                                         // and 1280 / 720 =~1.78, so this made sense to me
