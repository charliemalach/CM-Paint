 ...
 * This class file is used to manage the tools used to draw on the canvas. All
 * the tool methods and their getters / setters are defined here. INCLUDES
 * JAVADOC COMMENTING
 **/
public class PaintDraw extends Canvas { // extends the resizable canvas
     ...
     * @param y2 = final y
     */
    public void squareTool(double x1, double y1, double x2, double y2) { // draws a square to the canvas
        double x = (Math.min(x1, x2)); // set x to the smaller of the two values to map to bottom left
        double y = (Math.min(y1, y2));
        double height;
        double width = height = Math.abs(x1 - x2); // abs val of the two x's = length of x
        this.gc.strokeRect(x, y, width, height);
        logData(" user drew a square");
    }
     ...
     */
    public void polygonTool(double x1, double y1, double x2, double y2, int s) { // draws a polygon using parameters
        ...
        logData(" user drew a polygon");
    }

    /**
     * draws a triangle to the canvas of s sides (hard coded to 3) to the canvas
     * with a center at x1, y1 and calculates radius with x2, y2.
     * 
     ...
     */
    public void ellipseTool(double x1, double y1, double x2, double y2) { // draws an ellipse
     ...
     * @param y2 - y2 coordinate for final y
     */
    public void circleTool(double x1, double y1, double x2, double y2) { // draws a circle
        double x = (Math.min(x1, x2)); // calculates the minimum of x1 and x2
        double y = (Math.min(y1, y2)); // calculates the minimum of y1 and y2
        double height;
        double width = height = Math.abs(x1 - x2); // takes the absolute value of x1 - x2 (these will be the same
                                                   // because it is a circle)

        this.gc.strokeOval(x, y, width, height);
        logData(" user drew a circle");
    }
     ...
     * @param y2 - y2 coordinate for final y
     */
    public void dashedLineTool(double x1, double y1, double x2, double y2) { // draws a dashed line
        gc.setLineDashes(15); // sets dashes in the line
        gc.strokeLine(x1, y1, x2, y2); // draws dashed line from x1, y1 to x2, y2
        logData(" user drew a dashed line");
    }
     ...
     */
    public void drawPencilStart(double x1, double y1) { // draws the beginning of a freehand line
        gc.setLineDashes(0); // ensure there are no dashes in the pencil
        this.gc.setLineCap(StrokeLineCap.ROUND);
        this.gc.beginPath();
     ...
     */
    public void drawPencilEnd(double x1, double y1) { // draws the end of a freehand line
        gc.setLineDashes(0); // ensure there are no dashes in the pencil
        this.gc.setLineCap(StrokeLineCap.ROUND);
        this.gc.lineTo(x1, y1);
        this.gc.stroke();
        logData(" user drew with the pencil");
    }
     ...
     * @param y1 - y1 for initial y
     */
    public void drawEraserStart(double x1, double y1) { // draws the beginning of an eraser
        gc.setLineDashes(0);
        this.gc.setLineCap(StrokeLineCap.ROUND);
        this.gc.beginPath();
        this.gc.moveTo(x1, y1);
     ...
     * @param y1 - y1 for final y
     */
    public void drawEraserEnd(double x1, double y1) { // draws the end of an eraser
        gc.setLineDashes(0);
        this.gc.setLineCap(StrokeLineCap.ROUND);
        this.gc.lineTo(x1, y1);
        this.gc.stroke();
        logData(" user used the eraser");
    }
     ...
     */
    public void drawImage(Image im) { // clears the canvas and draws an image to the canvas
        clearCanvas();
        ...
        logData(" user drew an image to the canvas");
    }
    ...
    public void clearCanvas() {
        ...
        logData(" user cleared the canvas");
    }
    ...
    public void setLineColor(Color color) {
        gc.setStroke(color);
        logData(" user set the line color");
    }
    ...
    public void setFillColor(Color color) {
        gc.setFill(color);
        logData(" user set the fill color");
    }
     ...
     */
    public void setShapeFill(boolean shapeFill) // sets shape fill color
    {
