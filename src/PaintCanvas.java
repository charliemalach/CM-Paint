 ...
 * This class file is used to manage the changes made on the canvas. Tool
 * selection and their methods occurs here.
 *
 **/
    ...
    public PaintCanvas() {
        ...
        setOnMouseMoved(e -> { // when mouse is moved at all on canvas
            switch (PaintToolBar.getTool()) {
                case ("Line"), ("Dashed Line"), ("Rectangle"), ("Pencil"), ("Ellipse"), ("Square"), ("Triangle"),
                        ("Circle"), ("None"), ("Eraser"), ("Copy"), ("Cut") ->
                    setCursor(Cursor.DEFAULT); // cursor is just default
                case ("Clear Canvas"), ("Paste") -> setCursor(Cursor.OPEN_HAND); // cursor is hand, couldn't think of
                                                                                 // better cursor

                case ("Color Dropper") -> setCursor(Cursor.CROSSHAIR); // cursor is crosshair, thought it would be p
                                                                       // cool
            }
        });
        ...
        setOnMouseReleased(e -> {
            x1 = e.getX(); // new x param
            y1 = e.getY(); // new y param
            ...
            switch (PaintToolBar.getTool()) {
                ...
                case ("Dashed Line"): // draws a dashed line using the coordinates
                    this.lineDashes(0);
                    this.dashedLineTool(x, y, x1, y1); // draws a dashed line to new ending coordinates
                    this.updateCanvas();
                ...
                case ("Square"): // draws a square using the coordinates
                    this.lineDashes(0);
                    this.squareTool(x, y, x1, y1); // draws a square from the starting coordinates to the new ending
                                                   // coordinates
                    this.updateCanvas();
                ...
                case ("Rectangle"): // draws a rectangle using the coordinates
                    this.lineDashes(0);
                    this.rectTool(x, y, x1, y1); // draws a rectangle from the starting coordinates to the coordinates
                    this.updateCanvas();
                ...
                case ("Ellipse"): // draws an ellipse using the coordinates
                    this.lineDashes(0);
                    this.ellipseTool(x, y, x1, y1); // draws an ellipse from the starting coordinates to the coordinates
                    this.updateCanvas();
